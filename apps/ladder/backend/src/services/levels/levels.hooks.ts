...
const populatePosition = (options) => async (context: HookContext) => {
...
const orderByPosition = (options) => async (context: HookContext) => {
    context.params.query.$sort = { position: 1 };

    return context;
};

const reorder = (options) => async (context: HookContext) => {
