...
const populateTournament = (options) => async (context) => {
    ...
    const { year, season, level } = context.params.query;
        ...
        tournamentInfo.teams = result.reduce((arr, item) => {
            ...
            if (arr.length > 0 && arr[arr.length - 1].id === item.id) {
                ...
                arr.push({
                    ...
                    players: [player],
                    playingNextWeek: item.playingNextWeek ? item.playingNextWeek.split(',').map(Number) : [player.id],
                });
            }
        ...
        if (!hasFinalMatches) {
            ...
            if (isSingles) {
                ...
                (async () => {
                    ...
                    const bracketImage = await renderImage(`${TL_URL}/image/bracket?tournamentId=${tournamentInfo.id}`);
