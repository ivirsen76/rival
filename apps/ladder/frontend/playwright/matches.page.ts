...
(() => {
    ...
    test('Should autocomplete the address', async ({ page, common, match, overview }) => {
        const dateTwoDaysAgo = dayjs.tz().subtract(2, 'day').format('YYYY-MM-DD HH:mm:ss');
        await runQuery(`UPDATE players SET readyForFinal=1 WHERE userId!=2`);
        await runQuery(`UPDATE seasons SET endDate="${dateTwoDaysAgo}", isFree=1 WHERE id=1`);
        await overrideConfig({ minMatchesToPlanTournament: 2, minPlayersToRunTournament: 2 });
        ...
        await overview.streetField.fill('908 sutter');
        await overview.pickAddressOption('908 Sutter Gate Ln, Morrisville, NC, 27560');
        await expect(overview.streetField).toHaveValue('908 Sutter Gate Ln');
        await expect(overview.apartmentField).toHaveValue('');
        await expect(overview.cityField).toHaveValue('Morrisville');
        await expect(overview.stateField).toHaveValue('NC');
        await expect(overview.zipField).toHaveValue('27560');

        // check if leaving street address field will remove suggestions
        await overview.streetField.fill('105');
        await expect(page.locator('[data-address-autocomplete-option]').first()).toBeVisible();
        await overview.apartmentField.fill('101');
        await expect(page.locator('[data-address-autocomplete-option]').first()).toBeHidden();

        // check if we can pick option using keyboard
        await page.goto('/season/2021/spring/men-35');
        await overview.streetField.fill('105');
        await expect(page.locator('[data-address-autocomplete-option]').nth(2)).toBeVisible();
        const thirdOption = await page.locator('[data-address-autocomplete-option]').nth(2).textContent();
        const [street, city, state, zip] = thirdOption.split(', ');

        await page.keyboard.press('ArrowDown');
        await page.keyboard.press('ArrowDown');
        await page.keyboard.press('Enter');

        await expect(overview.streetField).toHaveValue(street);
        await expect(overview.cityField).toHaveValue(city);
        await expect(overview.stateField).toHaveValue(state);
        await expect(overview.zipField).toHaveValue(zip);

        // should save an address and calculate deliverability metric
        await overview.streetField.fill('326 Nelson St. SW');
        await overview.apartmentField.fill('#301');
        await overview.cityField.fill('Atlanta');
        await overview.stateField.selectOption('GA');
        await overview.zipField.fill('30313');
        await page.locator('button').getByText('Claim reward').click();

        await expect(overview.claimAwardArea).toBeHidden();

        // let address verification to be finished
        await page.waitForTimeout(3000);
        const info = await expectRecordToExist(
            'players',
            { id: 2 },
            { address: '326 Nelson St. SW #301, Atlanta, GA, 30313' }
        );
        expect(JSON.parse(info.addressVerification)).toMatchObject({
            addressLine1: '326 Nelson St Sw',
            addressLine2: 'Unit 301',
            city: 'Atlanta',
            state: 'GA',
            zip: '30313-1361',
            confidence: 1,
            result: 'Address verified successfully',
        });
    });

    test('Should get an address even if autocomplete service is not working', async ({
        page,
        common,
        match,
        overview,
        ...
        // let address verification attempt to be finished
        await page.waitForTimeout(3000);
        await expectRecordToExist(
            'players',
            { id: 2 },
            { address: '908 Sutter Gate Ln, Morrisville, NC, 27560', addressVerification: null }
        );
    });

    test('Should claim the award as a winner and choose a gift card for Raleigh', async ({
        page,
        common,
        overview,
        match,
        ...
        await runQuery(`UPDATE players SET readyForFinal=1 WHERE userId!=2`);
        await runQuery(`UPDATE seasons SET endDate="${dateTwoDaysAgo}" WHERE id=1`);
        await overrideConfig({ minMatchesToPlanTournament: 2, minPlayersToRunTournament: 2, isRaleigh: 1 });
        ...
        await page.locator('label').getByText('$50 Gift Card from Tennis Warehouse').click();
        await page.locator('button').getByText('Claim reward').click();

        await expect(common.modal).toContainText('Congratulations!');
        await expect(common.modal).toContainText(
            'Your digital gift card will arrive to your email within the next 24 hours.'
        );
        await expect(common.modal).toContainText(
            'You can pick up your trophy at Millbrook Exchange Tennis Center after'
        );
        await expect(common.modal).not.toContainText('credit');

        await expectRecordToExist('players', { id: 2 }, { address: '-', rewardType: 'gift' });
        const email = await expectRecordToExist('emails', { subject: 'The champion claimed their reward' });
        expect(email.html).toContain('Ben Done');
        expect(email.html).toContain('Raleigh');
        expect(email.html).toContain('Men 3.5');
    ...
    test('Should claim the award as a runner-up and choose a credit for Raleigh', async ({
        ...
        await page.locator('label').getByText('$30 Rival Credit (Most Popular - $5 Bonus!)').click();
        await page.locator('button').getByText('Claim reward').click();

        await expect(common.modal).toContainText('Congratulations!');
        await expect(common.modal).toContainText("You've received a $30 credit to your Rival Wallet.");
        await expect(common.modal).toContainText(
            'You can pick up your trophy at Millbrook Exchange Tennis Center after'
        );

        await expectRecordToExist('players', { id: 2 }, { address: '-', rewardType: 'credit' });
        const email = await expectRecordToExist('emails', { subject: 'The runner-up claimed their reward' });
        expect(email.html).toContain('Ben Done');
        expect(email.html).toContain('Raleigh');
        expect(email.html).toContain('Men 3.5');
    ...
    test('Should claim the award as a winner and choose a gift', async ({ page, common, overview, match }) => {
        ...
        await page.locator('label').getByText('Gift Card from Tennis Warehouse').click();
        await overview.streetField.fill('908 Sutter Gate Ln');
        await overview.cityField.fill('Morrisville');
        await overview.zipField.fill('27560');
        await page.locator('button').getByText('Claim reward').click();
        ...
        await expect(common.modal).toContainText('Congratulations!');
        await expect(common.modal).toContainText("You'll receive your trophy and gift card in 2-3 weeks.");
        await expect(common.modal).not.toContainText('credit');

        await expectRecordToExist(
            'players',
            { id: 2 },
            { address: '908 Sutter Gate Ln, Morrisville, NC, 27560', rewardType: 'gift' }
        );
        const email = await expectRecordToExist('emails', { subject: 'The champion claimed their reward' });
        expect(email.html).toContain('Ben Done');
        expect(email.html).toContain('Raleigh');
    ...
    test('Should claim the award as a winner and get a credit', async ({ page, common, overview, match }) => {
        ...
        await expect(overview.claimAwardArea).toBeVisible();
        await expect(common.body).toContainText('$55 Rival Credit');
        await expect(common.body).toContainText('$50 Gift Card');
        await expect(common.body).toContainText('engraved Trophy');
        await overview.streetField.fill('908 Sutter Gate Ln');
        await overview.cityField.fill('Morrisville');
        await overview.zipField.fill('27560');
        await page.locator('button').getByText('Claim reward').click();
        ...
        await expect(common.modal).toContainText('Congratulations!');
        await expect(common.modal).toContainText("You've received a $55 credit to your");
        await expect(common.modal).toContainText("You'll receive your trophy in 2-3 weeks.");
        await expect(overview.claimAwardArea).toBeHidden();
    ...
    test('Should claim the award as a runner-up and get a credit', async ({ page, common, overview, match }) => {
        ...
        await expect(overview.claimAwardArea).toBeVisible();
        await expect(common.body).toContainText('$30 Rival Credit');
        await expect(common.body).toContainText('$25 Gift Card');
        await expect(common.body).toContainText('engraved trophy');
        await overview.streetField.fill('908 Sutter Gate Ln');
        await overview.cityField.fill('Morrisville');
        await overview.zipField.fill('27560');
        ...
        await expect(common.modal).toContainText('Congratulations!');
        await expect(common.modal).toContainText("You've received a $30 credit to your");
        await expect(common.modal).toContainText("You'll receive your trophy in 2-3 weeks.");
        await expect(overview.claimAwardArea).toBeHidden();
    ...
    test('Should claim the award as a runner-up and get a gift card. Also strip commas from address parts.', async ({
        page,
        common,
        overview,
        match,
        ...
        await page.locator('label').getByText('Gift Card from Tennis Warehouse').click();
        await overview.streetField.fill('908 Sutter Gate, Ln');
        await overview.cityField.fill('Morri,sville');
        await overview.zipField.fill('27560');
        await page.locator('button').getByText('Claim reward').click();
